 ...
 */
import { downloadMpx, packageMpx, unpackMpx, MpxFormatError, type JsonValue } from '@/lib/mpx-packager';
import { newBlock, serializeBlock, isBlockReady, type BlockDraft } from '@/components/pages/block-draft';

interface MpxPageContent {
  readonly title: string;
  readonly content: { readonly schemaVersion: 1; readonly blocks: readonly Record<string, unknown>[] };
}
...
export interface MpxExportInput {
  ...
  readonly resolveFile: (
    fileId: string,
  ) => Promise<{ readonly ok: true; readonly file: File } | { readonly ok: false; readonly message: string }>;
}

export type MpxExportResult = { readonly ok: true } | { readonly ok: false; readonly message: string };

/** Packages a page's ready blocks and their PDF attachments into a downloadable .mpx archive. */
export async function exportPageAsMpx({ title, blocks, resolveFile }: MpxExportInput): Promise<MpxExportResult> {
  const ready = blocks.filter(isBlockReady);
  if (ready.some((block) => block.type === 'image')) {
    return { ok: false, message: 'MPX export only supports PDF attachments today - remove any image blocks first.' };
  }
  ...
  for (const block of ready) {
    ...
    if (resolved.file.type !== 'application/pdf') {
      return { ok: false, message: `"${block.label || 'A file block'}" is not a PDF - MPX export only supports PDF attachments.` };
    }
    ...
    attachments.push(new File([resolved.file], `${block.id}--${resolved.file.name}`, { type: resolved.file.type }));
  }
  ...
  try {
    ...
    return {
      ok: false,
      message: error instanceof MpxFormatError ? error.message : 'Could not package this page as MPX.',
    };
  }
}
...
export type MpxImportOutcome = MpxImportResult | { readonly ok: false; readonly message: string };
...
export async function importMpxFile(file: File): Promise<MpxImportOutcome> {
  ...
  try {
    ...
    return {
      ok: false,
      message: error instanceof MpxFormatError ? error.message : 'Could not read this MPX file.',
    };
  }

  const page = unpacked.page;
  if (page === null || typeof page !== 'object' || Array.isArray(page)) {
    return { ok: false, message: 'This MPX file does not contain a recognizable page.' };
  }
  const fields = page as unknown as Record<string, unknown>;
  const rawContent = fields.content as { blocks?: unknown } | undefined;
  if (typeof fields.title !== 'string' || typeof rawContent !== 'object' || rawContent === null) {
    return { ok: false, message: 'This MPX file does not contain a recognizable page.' };
  }
  const title = fields.title;
  const rawBlocks = Array.isArray(rawContent.blocks) ? rawContent.blocks : [];

  const filesByName = new Map(unpacked.attachments.map((attachment) => [attachment.name, attachment]));
  ...
  for (const raw of rawBlocks) {
    ...
    if (!block) continue;
    if (block.type === 'file') {
      const match = [...filesByName.entries()].find(([name]) => name.startsWith(`${block.id}--`));
      if (match) {
        const [name, attachedFile] = match;
        pendingFiles.set(block.id, new File([attachedFile], name.slice(block.id.length + 2), { type: attachedFile.type }));
      }
    }
    blocks.push(block);
  }
...
function parseImportedBlock(raw: unknown): BlockDraft | null {
  ...
  const id = typeof value.id === 'string' && value.id.trim() ? value.id : newBlock('paragraph').id;
  switch (value.type) {
    case 'paragraph':
      return { id, type: 'paragraph', html: typeof value.html === 'string' ? value.html : '' };
    case 'heading':
      return {
        id,
        type: 'heading',
    ...
    case 'callout':
      return {
        id,
        type: 'callout',
        tone: value.tone === 'info' || value.tone === 'warning' ? value.tone : 'neutral',
        title: typeof value.title === 'string' ? value.title : '',
        html: typeof value.html === 'string' ? value.html : '',
      };
    case 'file':
      return { id, type: 'file', fileId: '', label: typeof value.label === 'string' ? value.label : '', uploading: false };
    default:
