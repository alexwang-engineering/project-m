...
function parseResource(
  ...
  index: number,
  errors: string[],
): MigrationResource | null {
  if (!isRecord(raw)) {
    errors.push(`resources[${index}] must be an object.`);
    return null;
  }
  if (typeof raw.externalId !== 'string' || !raw.externalId)
    errors.push(`resources[${index}] needs a non-empty externalId.`);
  if (typeof raw.tagName !== 'string' || !raw.tagName)
    errors.push(`resources[${index}] needs a non-empty tagName.`);
  if (typeof raw.title !== 'string' || !raw.title)
    errors.push(`resources[${index}] needs a non-empty title.`);
  if (typeof raw.html !== 'string')
    errors.push(`resources[${index}] needs an html string (can be empty).`);
  if (
  ...
  return {
    ...
    html: raw.html,
    originalAuthor:
      typeof raw.originalAuthor === 'string' ? raw.originalAuthor : undefined,
  };
}
...
function parseAssignment(
  ...
  if (typeof raw.externalId !== 'string' || !raw.externalId)
    errors.push(`assignments[${index}] needs a non-empty externalId.`);
  if (typeof raw.tagName !== 'string' || !raw.tagName)
    errors.push(`assignments[${index}] needs a non-empty tagName.`);
  if (typeof raw.title !== 'string' || !raw.title)
    errors.push(`assignments[${index}] needs a non-empty title.`);
  if (
    raw.dueAt !== null &&
    raw.dueAt !== undefined &&
    typeof raw.dueAt !== 'string'
  )
    errors.push(`assignments[${index}].dueAt must be a string or null.`);
  if (typeof raw.allowResubmission !== 'boolean')
    errors.push(`assignments[${index}] needs allowResubmission as a boolean.`);
  if (
    typeof raw.externalId !== 'string' ||
    typeof raw.tagName !== 'string' ||
    typeof raw.title !== 'string' ||
    typeof raw.allowResubmission !== 'boolean'
  )
    return null;
  return {
    externalId: raw.externalId,
...
function parseQuiz(
  ...
  if (typeof raw.externalId !== 'string' || !raw.externalId)
    errors.push(`quizzes[${index}] needs a non-empty externalId.`);
  if (typeof raw.tagName !== 'string' || !raw.tagName)
    errors.push(`quizzes[${index}] needs a non-empty tagName.`);
  if (typeof raw.title !== 'string' || !raw.title)
    errors.push(`quizzes[${index}] needs a non-empty title.`);
  ...
  const questions = raw.questions.map((q, qi) => {
    if (
      ...
      errors.push(
        `quizzes[${index}].questions[${qi}] must have prompt, choices[], and correctChoiceIndex.`,
      );
      return null;
    }
...
export function parseMigrationManifest(text: string): {
  ...
  const resources = (Array.isArray(raw.resources) ? raw.resources : [])
    .map((r, i) => parseResource(r, i, errors))
    .filter((r): r is MigrationResource => r !== null);
  const assignments = (Array.isArray(raw.assignments) ? raw.assignments : [])
    .map((a, i) => parseAssignment(a, i, errors))
    .filter((a): a is MigrationAssignment => a !== null);
  const quizzes = (Array.isArray(raw.quizzes) ? raw.quizzes : [])
    .map((q, i) => parseQuiz(q, i, errors))
    .filter((q): q is MigrationQuiz => q !== null);
