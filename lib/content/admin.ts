...
export async function listUsers(
  ...
  return (data ?? []).map((row) => ({
    ...
    tagMemberships: row.tag_memberships
      .filter(
        (m) => isCurrentlyValid(m.valid_from, m.valid_until) && m.tags !== null,
      )
      .map((m) => ({
...
export async function assignTagMembership(
  client: Client,
  input: unknown,
): Promise<AdminActionResult> {
  const value = record(input);
...
export async function setProfileState(
  ...
  if (value.state !== 'active' && value.state !== 'disabled') {
    return {
      ok: false,
      code: 'invalid_input',
      message: 'State must be active or disabled.',
    };
  }
  if (typeof value.reason !== 'string' || !value.reason.trim()) {
    return {
...
export type CreateTagResult =
  | {
      readonly ok: true;
      readonly tag: { readonly id: string; readonly name: string };
    }
  | {
      readonly ok: false;
