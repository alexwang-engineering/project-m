...
async function resolveTagId(
  ...
  const { data } = await client
    .from('tags')
    .select('id')
    .eq('tag_name', tagName.toUpperCase())
    .eq('is_active', true)
    .maybeSingle();
  return data?.id ?? null;
}
...
async function checkExisting(
  ...
  | { readonly proceed: false; readonly status: 'unchanged' | 'conflict' }
> {
  const { data } = await client.rpc('get_migration_import', {
    ext_source: 'moodle',
    ext_id: externalId,
  });
  ...
  return {
    proceed: false,
    status: data.content_checksum === checksum ? 'unchanged' : 'conflict',
  };
}
  ...
  for (const resource of manifest.resources) {
    ...
    if (!tagId) {
      report.push({
        ...
        status: 'quarantined',
        message: `Tag "${resource.tagName}" does not exist.`,
      });
      continue;
    }
    ...
    const slug = `moodle-${resource.externalId}`
      .toLowerCase()
      .replace(/[^a-z0-9-]+/g, '-');
    const created = await createPage(client, {
      ...
      content: {
        schemaVersion: 1,
        blocks: [
          { id: crypto.randomUUID(), type: 'paragraph', html: resource.html },
        ],
      },
      tagIds: [tagId],
    });
  ...
  for (const assignment of manifest.assignments) {
    ...
    if (!tagId) {
      report.push({
        ...
        status: 'quarantined',
        message: `Tag "${assignment.tagName}" does not exist.`,
      });
      continue;
    }
    const checksum = await checksumOf({
  ...
  for (const quiz of manifest.quizzes) {
    ...
    if (!tagId) {
      report.push({
        ...
        title: quiz.title,
        status: 'quarantined',
        message: `Tag "${quiz.tagName}" does not exist.`,
      });
      continue;
    }
    const unsupported = quiz.questions.some(
      (q) =>
        q.choices.length < 2 ||
    ...
    if (quiz.questions.length === 0 || unsupported) {
      report.push({
        ...
        status: 'quarantined',
        message:
          'Only multiple-choice questions with at least 2 choices are supported.',
      });
      continue;
    }
    const checksum = await checksumOf({
    ...
    const created = await createQuiz(client, {
      ...
      questions: quiz.questions.map((q) => {
        const choices = q.choices.map((label) => ({
