...
export async function listBankItems(
  client: Client,
): Promise<readonly BankItemSummary[]> {
  const { data, error } = await client
    .from('question_bank_items')
    .select(
      'id, prompt, choices, correct_choice_id, question_bank_item_tags(tags!inner(tag_name, display_name))',
    )
...
export async function createBankItem(
  ...
  if (!value)
    return {
      ok: false,
      code: 'invalid_input',
      message: 'Bank item input must be an object.',
    };
  ...
  if (
    ...
    return {
      ok: false,
      code: 'invalid_input',
      message: 'Between 2 and 8 choices are required.',
    };
  }
  if (typeof value.correctChoiceId !== 'string' || !value.correctChoiceId) {
    return {
      ok: false,
      code: 'invalid_input',
      message: 'A correct choice is required.',
    };
  }
  if (
  ...
  const { data, error } = await client.rpc('create_bank_item', {
    item_prompt: prompt,
    item_choices: JSON.parse(
      JSON.stringify(value.choices),
    ) as Database['public']['Functions']['create_bank_item']['Args']['item_choices'],
  ...
  if (error || !data) {
    ...
    if (code === '22023')
      return {
        ok: false,
        code: 'invalid_input',
        message: errorMessage(error, 'Invalid bank item details.'),
      };
    return {
      ok: false,
      code: 'failed',
      message: errorMessage(error, 'The bank item could not be created.'),
    };
  }
...
export async function archiveBankItem(
  client: Client,
  input: unknown,
  ...
  if (!value || typeof value.itemId !== 'string' || !UUID.test(value.itemId)) {
    return {
      ok: false,
      code: 'invalid_input',
      message: 'Bank item ID is invalid.',
    };
  }
  ...
  if (code === 'P0002')
    return {
      ok: false,
      code: 'not_found',
      message: 'The bank item was not found.',
    };
  return {
    ok: false,
    code: 'failed',
    message: 'The bank item could not be archived.',
  };
}
