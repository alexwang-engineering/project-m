...
export async function listAnnouncements(
  ...
  const { data, error } = await client
    .from('announcements')
    .select(
      'id, title, body, is_broadcast, created_by, created_at, announcement_tags(tag_id, tags!inner(tag_name))',
    )
...
export async function createAnnouncement(
  ...
  if (!value)
    return {
      ok: false,
      code: 'invalid_input',
      message: 'Announcement input must be an object.',
    };
  ...
  if (!value.broadcast && tagIds.length < 1) {
    return {
      ...
      code: 'invalid_input',
      message:
        'At least one audience tag is required for a non-broadcast announcement.',
    };
  }
  if (value.broadcast && tagIds.length > 0) {
    return {
      ok: false,
      code: 'invalid_input',
      message: 'A whole-school announcement must not list audience tags.',
    };
  }
  ...
  if (error || !data) {
    ...
    if (code === '42501') {
      return {
        ok: false,
        code: 'forbidden',
        message: 'You are not authorized to post this announcement.',
      };
    }
    if (code === '22023') {
      return {
...
export async function cancelAnnouncement(
  ...
  if (
    ...
    !UUID.test(value.announcementId)
  ) {
    return {
      ok: false,
      code: 'invalid_input',
  ...
  if (code === '42501')
    return {
      ok: false,
      code: 'forbidden',
      message: 'You do not manage this announcement.',
    };
  if (code === 'P0002')
    return {
      ok: false,
  ...
  return {
    ok: false,
    code: 'failed',
    message: 'The announcement could not be cancelled.',
  };
}
