...
export function parseRosterCsv(text: string): {
  ...
  if (emailIdx === -1 || roleIdx === -1 || tagsIdx === -1) {
    return {
      rows: [],
      parseErrors: [
        'Header row must contain email, systemRole, and tags columns.',
      ],
    };
  }
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (systemRole !== 'student' && systemRole !== 'teacher') {
      parseErrors.push(
        `Line ${lineNumber}: systemRole must be "student" or "teacher".`,
      );
      continue;
    }
    const tagsCell = cells[tagsIdx] ?? '';
    const memberships: RosterMembership[] = [];
    for (const part of tagsCell
      .split(';')
      .map((p) => p.trim())
      .filter((p) => p.length > 0)) {
      ...
      if (
        ...
        parseErrors.push(
          `Line ${lineNumber}: invalid role "${membershipRole}" for tag ${tagName}.`,
        );
        continue;
      }
